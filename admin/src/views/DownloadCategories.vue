...
async function load() {
  loading.value = true;
  try {
    const data = await api.downloadCategories.list({ page: 1, size: 100 });
...
async function save() {
  ...
  saving.value = true;
  try {
    if (editingId.value) await api.downloadCategories.update(editingId.value, form);
    else await api.downloadCategories.create(form);
...
async function remove(row: any) {
  ...
  await api.downloadCategories.remove(row.id);
...
<template>
  <div class="page">
    <div class="page-toolbar">
      <span class="page-title">资料分类</span>
